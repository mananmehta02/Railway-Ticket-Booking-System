...
int login(int sock);
int signup(int sock);
int AdminMenu(int sock, int id);
int UserMenu(int sock, int id, int type);
void ViewBooking(int sock, int id, int type);
	...
	if(type == 1){
		...
		if(temp.id == acc_no){
			if(!strcmp(temp.pass, password)){
				...
				while(-1!=UserMenu(sock, temp.id, type));
				login_success = 1;
			}
		}
		lock.l_type = F_UNLCK;
	...
	// Type 3 is for admin
		...
		if(ret != -1){
			...
			if(temp.id == acc_no){
				if(!strcmp(temp.pass, password)){
					write(sock, &valid, sizeof(valid));
					while(-1!=AdminMenu(sock, temp.id));
					login_success = 1;
				}
			}
			lock.l_type = F_UNLCK;
...
int AdminMenu(int sock, int id){
	...
	if(op_id == 1){
		...
		read(sock,&seatsavl,sizeof(seatsavl));
		struct train temp, temp2;

		temp.tid = tid;
		temp.train_no = tno;
		strcpy(temp.train_name, tname);
		temp.av_seats = seatsavl;

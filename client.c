...
int UserMenu(int sock, int type);
int AdminMenu(int sock, int action);
int UserMenu2(int sock, int opt);
void ViewBooking(int sock);
...
int MainMenu(int sock){
	...
	if(opt==1){
		...
		printf("Select any account type:\n");
		printf("1. User \n2. Agent\n3. Admin\n");
...
int UserMenu(int sock, int type){
	...
	if(type == 1 || type == 2){
		...
		printf("Your Choice: ");
		scanf("%d", &opt);
		return AdminMenu(sock, opt);
	}
}

int AdminMenu(int sock, int opt){
	switch(opt){
		case 1:{
			int tno,seatsavl;
			char tname[20];
			...
			printf("Enter Available Seats: ");scanf("%d",&seatsavl);
			write(sock, &tname, sizeof(tname));
			write(sock, &tno, sizeof(tno));
			write(sock,&seatsavl,sizeof(seatsavl));
...
void ViewBooking(int sock){
	...
	while(entries--){
		...
		if(!cancelled)
		printf("BookingID: %d\tFirs Ticket: %d\tLast Ticket: %d\tTRAIN :%s\n", bid, bks_seat, bke_seat, trainname);
	}
	printf("Press any key to continue...\n");
	while(getchar()!='\n');
	getchar();
